...
example. Finally, run the command \cft{runCompswitch(1,true)}, which should
generate the plot shown in Fig.\ \ref{fig_1}.
\begin{figure}[ht]
\centering
...
(\cft{am\_queue}), where they wait, until the server (\cft{am\_serverDist}) is
...
output values of the generator, the queue and the server. The only distribution
parameter is set to $0.9$, it defines the mean service time of the server. As
always, the output blocks have different and meaningful names for their
\cft{varname}. And just to get nicer looking plots, the parameter \cft{id of
  the first entity} of the generator is set to $1$.
...
for a short period after $t = 5$, the queue is empty before $t = 10$.
...
$t_S = 1.1$. It is delayed by the \cft{am\_serverDist} and leaves the
